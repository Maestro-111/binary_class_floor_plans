...
def load_dataset_train_preds_chain(load_flag:bool,prep_flag:bool,train_test_flag:bool,preds_flag:bool,holder):
    ...
    if preds_flag:
        ...
        make_preds(class_names, model_path='model.h5', path_for_floor_pdfs=floor_plan,
...
def main(train_save:int, test:int, training_image_data:str, testing_image_data:str):
    ...
    holder = DataHolder() # to hold var paths during the run

    if strategy in [0,2] and (not training_image_data): # if we want to do training and do now have the path for training data
        raise CustomException("Unspecified env var for training\n")

    if strategy != 1: # set the names of training paths into holder object
    ...
    if strategy == 0: # full pipeline
        ...
        while True:
            try:
                choice = int(input("Do you want to clear the folders (1 - Yes, 0 - No): "))
    ...
    else: # train and process witout data loading
        raise CustomException("Strategy is unknown\n")


parser = argparse.ArgumentParser()

parser.add_argument("--train_save", help='load and prep data, then save the trained model', action='store_true')
parser.add_argument("--test", help='predict for new images', action='store_true')

parser.add_argument("--image_training_data", help = 'name of the training directory', default=os.getenv('image_training_data')) # def - env var
parser.add_argument("--image_testing_data", help = 'name of the testing directory', default=os.getenv('image_testing_data'))
